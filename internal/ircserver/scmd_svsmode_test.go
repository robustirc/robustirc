...
func TestServerSvsmode(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("MODE secure")),
		":sECuRE!blah@robust/0x13b5aa0a2bcfb8ad MODE sECuRE :+")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["services"], irc.ParseMessage("SVSMODE secure +r")),
		":services.robustirc.net MODE sECuRE :+r")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["services"], irc.ParseMessage("SVSMODE socoro +r")),
		":robustirc.net 401 * socoro :No such nick/channel")

	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["services"], irc.ParseMessage("SVSMODE secure +rq")),
		[]*irc.Message{
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["services"], irc.ParseMessage("SVSMODE secure +d-r")),
