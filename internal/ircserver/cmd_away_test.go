...
func TestAway(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("PRIVMSG mero :hey")),
		":sECuRE!blah@robust/0x13b5aa0a2bcfb8ad PRIVMSG mero :hey")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("AWAY :upgrading server")),
		":robustirc.net 306 mero :You have been marked as being away")

	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("PRIVMSG mero :you there?")),
		[]*irc.Message{
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("AWAY")),
