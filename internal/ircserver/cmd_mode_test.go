...
func TestChannelMode(t *testing.T) {
	...
	i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test -nt"))
	i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("JOIN #test"))

	// Set a topic from the outside.
	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("TOPIC #test :foobar")),
		[]*irc.Message{
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test +t")),
		":mero!foo@robust/0x13b5aa0a2bcfb8ae MODE #test +t")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("TOPIC #test :bleh")),
		":robustirc.net 482 sECuRE #test :You're not channel operator")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("MODE #test")),
	...
	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("TOPIC #test :bleh")),
		[]*irc.Message{
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test +o sECuRE")),
		":mero!foo@robust/0x13b5aa0a2bcfb8ae MODE #test +o sECuRE")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test +o nobody")),
		":robustirc.net 441 mero nobody #test :They aren't on that channel")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test +y sECuRE")),
		":robustirc.net 472 mero y :is unknown mode char to me")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #nonexistant +y sECuRE")),
		":robustirc.net 442 mero #nonexistant :You're not on that channel")

	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("TOPIC #test :finally")),
		[]*irc.Message{
...
func TestUserMode(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("MODE sECuRE +i")),
		":sECuRE!blah@robust/0x13b5aa0a2bcfb8ad MODE sECuRE :+i")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("MODE sECuRE -i")),
		":robustirc.net 502 xeen :Can't change mode for other users")

	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("OPER xeen foo")),
		[]*irc.Message{
...
func TestBans(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("MODE #test b")),
...
func TestChannelMemberStatus(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["mero"], irc.ParseMessage("MODE #test +o xeen")),
		":mero!foo@robust/0x13b5aa0a2bcfb8ae MODE #test +o xeen")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("MODE #test -o+o xeen sECuRE")),
		":xeen!baz@robust/0x13b5aa0a2bcfb8af MODE #test +o-o sECuRE xeen")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("MODE #test +o xeen")),
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("TOPIC #test :nooo")),
		":robustirc.net 482 xeen #test :You're not channel operator")
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("MODE xeen")),
...
func TestInvisible(t *testing.T) {
	...
	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("PRIVMSG sECuRE :before invisible")),
	...
	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("PRIVMSG sECuRE :after invisible")),
		nil)

	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("NOTICE sECuRE :after invisible")),
		nil)

	i.ProcessMessage(robust.Id{}, ids["secure"], irc.ParseMessage("JOIN #common"))
	i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("JOIN #common"))

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("PRIVMSG sECuRE :common channel")),
		":xeen!baz@robust/0x13b5aa0a2bcfb8af PRIVMSG sECuRE :common channel")

	mustMatchMsg(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("NOTICE sECuRE :common channel")),
...
func TestDefaultChannelModes(t *testing.T) {
	...
	mustMatchIrcmsgs(t,
		i.ProcessMessage(robust.Id{}, ids["xeen"], irc.ParseMessage("JOIN #foobar")),
		[]*irc.Message{
