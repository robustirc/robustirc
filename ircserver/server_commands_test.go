...
func TestServerSvshold(t *testing.T) {
	...
	mustMatchIrcmsgs(t,
		i.ProcessMessage(ids["services"], irc.ParseMessage("SVSHOLD newnick 5 :held by services")),
		[]*irc.Message{})

	mustMatchMsg(t,
		i.ProcessMessage(ids["secure"], irc.ParseMessage("NICK newnick")),
	...
	mustMatchIrcmsgs(t,
		i.ProcessMessage(ids["services"], irc.ParseMessage("SVSHOLD anothernick 5 :held by services")),
		[]*irc.Message{})

	mustMatchMsg(t,
		i.ProcessMessage(ids["secure"], irc.ParseMessage("NICK anothernick")),
		":robustirc.net 432 newnick anothernick :Erroneous Nickname: held by services")

	mustMatchIrcmsgs(t,
		i.ProcessMessage(ids["services"], irc.ParseMessage("SVSHOLD anothernick")),
		[]*irc.Message{})
